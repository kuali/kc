...
VALUES ((SELECT (MAX(ID)) FROM KRIM_ROLE_MBR_ID_BS_S), (SELECT ROLE_ID FROM KRIM_ROLE_T WHERE NMSPC_CD = 'KC-COIDISCLOSURE' AND ROLE_NM = 'COI Reporter'), (SELECT ROLE_ID FROM KRIM_ROLE_T WHERE NMSPC_CD = 'KUALI' AND ROLE_NM = 'User'), 'R', NOW(), UUID(), 1)
/
DELIMITER ;
