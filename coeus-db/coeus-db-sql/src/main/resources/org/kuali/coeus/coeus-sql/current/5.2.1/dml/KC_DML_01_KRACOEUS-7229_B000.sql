...
buffer := 'xsl:with-param>
									...
									<fo:table-row font-size="10pt" font-weight="bold">
										<fo:table-cell number-columns-spanned="2" border="sol';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE data CLOB; buffer VARCHAR2(30000);
...
WHERE
TEMPLATE_TYPE_CODE=4 AND  FORM_ID = 'FDP Template' FOR UPDATE;
buffer := 'id 1pt gray" padding="0" display-align="center">
											<fo:block>
												<fo:inline-container>
													<fo:block>
														<xsl:text>&#x2029;</xsl:text>
														...
														<fo:external-graphic content-height="8" content-width="8">
															<xsl:attribute name="src">
																...
																<xsl:call-template name="double-backslash">
																	...
																	<xsl:with-param name="text-length">
																		<xsl:value-of select="string-length(string(concat(subcontract:SubContractData/subcontract:PrintRequirement/subcontract:ImageCheckedPath,&apos;checked.gif&a';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE data CLOB; buffer VARCHAR2(30000);
...
buffer := '
												...
												<fo:basic-link text-decoration="underline" color="blue">
													<xsl:choose>
														...
														<xsl:otherwise>
															<xsl:attribute name="external-destinat';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE data CLOB; buffer VARCHAR2(30000);
...
WHERE
TEMPLATE_TYPE_CODE=4 AND  FORM_ID = 'FDP Template' FOR UPDATE;
buffer := '						<fo:inline font-family="Arial" font-size="9pt">
