CREATE OR REPLACE package s2sSF424V2Pkg as
...
procedure getAreasAffected
          (as_proposal_number IN osp$eps_proposal.proposal_number%type,
           cur_type IN OUT result_sets.cur_generic);
           
procedure getSF424V2ApplicantType
...
procedure getOrganization
 ...
 procedure getCongDist
         (as_proposal_number IN osp$eps_proposal.proposal_number%type,
         as_organizationID IN osp$eps_prop_sites.organization_id%type,
         ai_type IN number,
          cur_type IN OUT result_sets.cur_generic) ;
...
CREATE OR REPLACE package body s2sSF424V2Pkg as
...
procedure getType
	(as_proposal_number in osp$eps_proposal.proposal_number%type,
	 cur_type IN OUT result_sets.cur_generic)
is
ls_SF424V2ApplicationType varchar2(200);
ls_submissionType_code varchar2(5);
ls_submissionType_desc varchar2(200);
ls_revision_code varchar2(5);
ls_revision_desc varchar2(200);
ls_fedid varchar2(30) := '';  --case 3146

begin
 	s2spackage.getSF424ApplicationType(as_proposal_number,cur_type);

 
 	fetch cur_type into ls_SF424V2ApplicationType;
 	IF cur_type%NOTFOUND THEN
	  ls_SF424V2ApplicationType := ' ';
	END IF;
	
	s2spackage.getSubmissionType(as_proposal_number,cur_type);
	fetch cur_type into ls_submissionType_code,ls_submissionType_desc,ls_revision_code,ls_revision_desc;
	IF cur_type%NOTFOUND THEN
		ls_submissionType_code := ' ';
	END IF;

	open cur_type for
	  select decode(ls_SF424V2ApplicationType,'N','New',
	  					  'C','Continuation',
	  					  'R','Revision',
	      					  'Unknown') as APPLICATIONTYPE,
	  ls_submissionType_code as SUBMISSION_TYPE_CODE,
	  decode(ls_submissionType_desc,'Change/Corrected Application','Changed/Corrected Application',ls_submissionType_desc) AS SUBMISSION_TYPE_DESC ,
	  decode(ls_revision_code,'A', 'A: Increase Award',
	  			  'B', 'B: Decrease Award',
	  			  'C', 'C: Increase Duration',
	  			  'D', 'D: Decrease Duration',
	  			  'E', 'E: Other (specify)',
	  			  'AC','AC: Increase Award, Increase Duration',
	  			  'AD','AD: Increase Award, Decrease Duration',
	  			  'BC','BC: Decrease Award, Increase Duration',
	  			  'BD','BD: Decrease Award, Decrease Duration') AS REVISION_TYPE,
	  ls_revision_desc AS REVISION_OTHER_DESC,
     ls_fedid as FEDID
	  from 	 dual;
...
begin
open cur_type for
 select	decode(as_ORG_TYPE_CODE, 1, 'C: City or Township Government',
				2, 'A: State Government',
				3, 'X: Other (specify)', --Federal Government
				4, 'M: Nonprofit with 501C3 IRS Status (Other than Institution of Higher Education)',
				5, 'N: Nonprofit without 501C3 IRS Status (Other than Institution of Higher Education)',
				6, 'Q: For-Profit Organization (Other than Small Business)',
				7, 'X: Other (specify)',
				8, 'I: Indian/Native American Tribal Government (Federally Recognized)',
				9, 'P: Individual',
				10, 'O: Private Institution of Higher Education',
				11, 'R: Small Business',
				14, 'X: Other (specify)', --socially and Economically Disadvantaged
				15, 'X: Other (specify)', --Women owned
				21, 'H: Public/State Controlled Institution of Higher Education',
				22, 'B: County Government',
				23, 'D: Special District Government',
				24, 'G: Independent School District',
				25, 'L: Public/Indian Housing Authority',
				26, 'J: Indian/Native American Tribal Government (Other than Federally Recognized)',
...
open cur_type for
select ls_cong_Dist as congDist
from dual;
...
begin
		BEGIN
			...
			CASE   	as_state_code
                        WHEN 'AL' THEN ls_string := 'AL: Alabama';
