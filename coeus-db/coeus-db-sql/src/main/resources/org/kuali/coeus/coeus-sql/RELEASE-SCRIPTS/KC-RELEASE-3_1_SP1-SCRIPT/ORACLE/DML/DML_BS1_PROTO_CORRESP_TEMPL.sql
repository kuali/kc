...
Insert into PROTO_CORRESP_TEMPL (PROTO_CORRESP_TEMPL_ID,PROTO_CORRESP_TYPE_CODE,COMMITTEE_ID,FILE_NAME,CORRESPONDENCE_TEMPLATE,UPDATE_TIMESTAMP,UPDATE_USER,OBJ_ID) values (SEQ_PROTO_CORRESP_TEMPL.nextVal, 10,'DEFAULT','10-ScheduleMinutes.xslt',EMPTY_CLOB(),sysdate,user,sys_guid());
DECLARE    data CLOB; buffer VARCHAR2(30000);
BEGIN
SELECT CORRESPONDENCE_TEMPLATE INTO data FROM PROTO_CORRESP_TEMPL
WHERE
...
Guests</fo:inline><fo:list-block provisional-distance-between-starts="7mm" provisional-label-separation="2mm" start-indent="2mm" space-before.optimum="4pt" space-after.optimum="4pt">
                            ...
                            <xsl:if test="n1:Attendents/n1:PresentFla';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE    data CLOB; buffer VARCHAR2(30000);
...
WHERE
PROTO_CORRESP_TYPE_CODE =  10 and COMMITTEE_ID='DEFAULT'   FOR UPDATE;
buffer := 'g =&apos;false&apos;"><fo:inline font-weight="bold">
								...
								<!-- case 893 - remove Withdrawn protocols  -->
									...
									<fo:table width="100%" space-before.optimum="1pt" space-after.optimum="2pt">
										...
										<fo:table-body>
											...
											<fo:table-row>
												<fo:table-cell display-align="before" width="10';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE    data CLOB; buffer VARCHAR2(30000);
...
WHERE
PROTO_CORRESP_TYPE_CODE =  10 and COMMITTEE_ID='DEFAULT'   FOR UPDATE;
buffer := '9pt" padding-start="3pt" padding-end="3pt" padding-before="3pt" padding-after="3pt" text-align="start" border-style="solid" border-width="1pt" border-color="white">
													<fo:block>
...
WHERE
PROTO_CORRESP_TYPE_CODE =  10 and COMMITTEE_ID='DEFAULT'   FOR UPDATE;
buffer := '										<xsl:if test="n1:VotingComments != ''null''">
...
/
set define off;
Insert into PROTO_CORRESP_TEMPL (PROTO_CORRESP_TEMPL_ID,PROTO_CORRESP_TYPE_CODE,COMMITTEE_ID,FILE_NAME,CORRESPONDENCE_TEMPLATE,UPDATE_TIMESTAMP,UPDATE_USER,OBJ_ID) values (SEQ_PROTO_CORRESP_TEMPL.nextVal, 9,'DEFAULT','9-AgendaReport.xslt',EMPTY_CLOB(),sysdate,user,sys_guid());
DECLARE    data CLOB; buffer VARCHAR2(30000);
BEGIN
SELECT CORRESPONDENCE_TEMPLATE INTO data FROM PROTO_CORRESP_TEMPL
WHERE
...
WHERE
PROTO_CORRESP_TYPE_CODE =  9 and COMMITTEE_ID='DEFAULT'   FOR UPDATE;
buffer := ' select="n1:ScheduleMasterData">
									<xsl:for-each select="n1:Place">
										<fo:inline-container>
											<fo:block>
												<xsl:text>&#x2029;</xsl:text>
...
WHERE
PROTO_CORRESP_TYPE_CODE =  9 and COMMITTEE_ID='DEFAULT'   FOR UPDATE;
buffer := '											<fo:inline font-weight="bold">
