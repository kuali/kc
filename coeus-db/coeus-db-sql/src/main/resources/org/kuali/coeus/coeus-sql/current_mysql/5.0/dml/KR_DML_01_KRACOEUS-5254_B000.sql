...
VALUES((SELECT (MAX(ID)) FROM KRIM_PERM_ID_BS_S),(SELECT PERM_TMPL_ID FROM KRIM_PERM_TMPL_T WHERE NMSPC_CD = 'KR-NS' AND NM = 'Open Document'),
...
VALUES((SELECT (MAX(ID)) FROM KRIM_ATTR_DATA_ID_BS_S),(SELECT PERM_ID FROM KRIM_PERM_T WHERE NM = 'View Sponsor' AND NMSPC_CD='KC-UNT'),
