...
BEGIN
  has_right := 1;--FN_USER_HAS_PROP_RIGHT(AS_USER_ID,AS_PROPOSAL_NUMBER,'MODIFY_BUDGET');
  if(has_right=0) then
  	     has_right := 1;--FN_USER_HAS_PROP_RIGHT(AS_USER_ID,AS_PROPOSAL_NUMBER,'VIEW_BUDGET');
  end if;
  return has_right;
  exception
  		  when others then
