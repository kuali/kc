...
PROTO_CORRESP_TYPE_CODE=(SELECT PROTO_CORRESP_TYPE_CODE FROM PROTO_CORRESP_TYPE WHERE DESCRIPTION = 'Approval Letter')  AND  COMMITTEE_ID =  'DEFAULT'
/
commit
/
...
buffer := 'd">
												...
												<fo:table-cell display-align="before" number-columns-spanned="5"  padding-start="3pt" padding-end="3pt" padding-before="3pt" padding-after="3pt" text-align="start" border-style="solid" border-width="1pt" border-color="white">
                                                    <fo:block>                                                          
														<xsl:for-each select="n1:Protocol">
                                                                        <xsl:for-each select="n1:RiskLevels">
                                                                            <xsl:for-each select="n1:RiskLevelDescription">																	
															<xsl:variable name="value-of-template">
                          ';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
...
buffer := '                                                          <xsl:apply-templates/>
                                ...
                                <xsl:choose>
                                    <xsl:when test="n1:Protocol/n1:Submissions/n1:CurrentSubmissionFlag = &apos;No&apos; and  n1:Protocol/n1:Submissions/n1:SubmissionDetails/n1:SubmissionTypeCode = 102">
                                        ...
                                        <fo:block/>
                                        <fo:inline font-family="Arial" font-size="10pt">
                                            <xsl:text>It is the Principal Investigator&apos;s responsibility to obtain review and continued approval before the expiration date.&#160; Please allow sufficient time for continued approval.&#160; You may not continue any research activity beyond the expiration date without COUHES approval.&#160; Failure to receive approval for continuation before the expiration date will result in the auto';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE    data CLOB; buffer VARCHAR2(30000);
...
WHERE
PROTO_CORRESP_TYPE_CODE=(SELECT PROTO_CORRESP_TYPE_CODE FROM PROTO_CORRESP_TYPE WHERE DESCRIPTION = 'Approval Letter')  AND  COMMITTEE_ID =  'DEFAULT' FOR UPDATE;
buffer := 'matic suspension of the approval of this protocol.&#160; Information collected following suspension is unapproved research and cannot be reported or published as research data.&#160; If you do not wish continued approval, please notify the Committee of the study termination.</xsl:text>
                                        </fo:inline>
                                    </xsl:when>
...
WHERE
PROTO_CORRESP_TYPE_CODE=(SELECT PROTO_CORRESP_TYPE_CODE FROM PROTO_CORRESP_TYPE WHERE DESCRIPTION = 'Approval Letter')  AND  COMMITTEE_ID =  'DEFAULT' FOR UPDATE;
buffer := '                  <fo:inline font-family="Arial" font-size="10pt">
                                            <xsl:text>.</xsl:text>
                                        </fo:inline>
                                        <fo:block/>
                                        <fo:inline font-family="Arial" font-size="10pt">
