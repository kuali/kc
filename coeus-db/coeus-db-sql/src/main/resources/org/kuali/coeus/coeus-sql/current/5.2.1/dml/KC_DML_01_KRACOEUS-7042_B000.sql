...
WHERE
TEMPLATE_TYPE_CODE=4 AND  FORM_ID = 'FDP Modification' FOR UPDATE;
buffer := '	<xsl:for-each select="subcontract:Address1">
																					<xsl:variable name="value-of-template">
																						<xsl:apply-templates/>
																					</xsl:variable>
																					<xsl:choose>
...
END;
/
INSERT INTO SUBAWARD_FORMS(FORM_ID,DESCRIPTION,UPDATE_TIMESTAMP,UPDATE_USER,FORM,FILE_NAME,CONTENT_TYPE,VER_NBR,OBJ_ID,TEMPLATE_TYPE_CODE) values ('FDP Template','FDP Agreement',SYSDATE,'admin',EMPTY_CLOB(),'FDP Agreement.xsl','application/octet-stream',1,SYS_GUID(),4)
/
DECLARE data CLOB; buffer VARCHAR2(30000);
BEGIN
SELECT FORM INTO data FROM SUBAWARD_FORMS
...
WHERE
TEMPLATE_TYPE_CODE=4 AND  FORM_ID = 'FDP Template' FOR UPDATE;
buffer := '											<xsl:for-each select="subcontract:RequisitionerOrgDetails">
																				<xsl:for-each select="subcontract:OrganizationName">
																					<xsl:variable name="value-of-template">
																						<xsl:apply-templates/>
																					</xsl:variable>
...
WHERE
TEMPLATE_TYPE_CODE=4 AND  FORM_ID = 'FDP Template' FOR UPDATE;
buffer := 'l:copy-of select="$value-of-template"/>
																								</fo:inline>
																							</xsl:otherwise>
																						</xsl:choose>
																					</xsl:for-each>
...
WHERE
TEMPLATE_TYPE_CODE=4 AND  FORM_ID = 'FDP Template' FOR UPDATE;
buffer := '	<xsl:text> See Attachment 4</xsl:text>
																			</fo:inline>
																		</xsl:when>
...
WHERE
TEMPLATE_TYPE_CODE=4 AND  FORM_ID = 'FDP Template' FOR UPDATE;
buffer := '="2" border="solid 1pt gray" padding="0" display-align="center">
											<fo:block>
												<fo:inline-container>
													<fo:block>
														<xsl:text>&#x2029;</xsl:text>
...
WHERE
TEMPLATE_TYPE_CODE=4 AND  FORM_ID = 'FDP Template' FOR UPDATE;
buffer := '									<xsl:for-each select="subcontract:ChangeRequestsContactDescription">
															<xsl:variable name="value-of-template">
																<xsl:apply-templates/>
															</xsl:variable>
															<xsl:choose>
...
WHERE
TEMPLATE_TYPE_CODE=4 AND  FORM_ID = 'FDP Template' FOR UPDATE;
buffer := '-cell padding="2pt" display-align="center">
											<fo:block>
												<fo:inline-container>
													<fo:block>
														<xsl:text>&#x2029;</xsl:text>
...
WHERE
TEMPLATE_TYPE_CODE=4 AND  FORM_ID = 'FDP Template' FOR UPDATE;
buffer := 'th-param name="text" select="$text-after-bs"/>
					<xsl:with-param name="text-length" select="$text-after-bs-length"/>
				</xsl:call-template>
			</xsl:otherwise>
		</xsl:choose>
...
buffer := 'value="1"/>.</fo:block>
														...
														<fo:list-item-body start-indent="body-start()">
															<fo:block font-family="time" font-size="10pt" font-weight="normal">
																...
																<fo:list-block provisional-distance-between-starts="7mm" provisional-label-separation="2mm">
																	<fo:list-item>
																		<fo:list-item-label end-indent="label-end()" text-alig';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE data CLOB; buffer VARCHAR2(30000);
								...
								<fo:table-body start-indent="0pt">
									<fo:table-row>
										<fo:table-cell font-family="times" font-size="12pt" font-weight="bold" padding="2pt" display-align="center">
											<fo:block>
												<fo:inline>
													<xsl:text>Special terms and conditio';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE data CLOB; buffer VARCHAR2(30000);
...
WHERE
TEMPLATE_TYPE_CODE=2 AND  FORM_ID = 'FDP_AMRMC' FOR UPDATE;
buffer := '		<xsl:when test="substring($text, $text-length) = ''\''">
						<xsl:value-of select="concat(substring($text,1,$text-length - 1), ''\\'')"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="$text"/>
...
WHERE
TEMPLATE_TYPE_CODE=3 AND  FORM_ID = 'FDP_ATT_3A' FOR UPDATE;
buffer := '			<xsl:for-each select="subcontract:OrgRolodexDetails">
...
WHERE
TEMPLATE_TYPE_CODE=3 AND  FORM_ID = 'FDP_ATT_3A' FOR UPDATE;
buffer := 'padding="2pt" display-align="center">
																<fo:block>
																	<fo:inline-container>
																		<fo:block>
																			<xsl:text>&#x2029;</xsl:text>
																			...
																			<fo:table-row>
																				<fo:table-cell padding="2pt" display-align="center">
																					<fo:block>
																						...
																						<xsl:for-each select="subcontract:SubContractData">
																							<xsl:for-each select="subcontract:P';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE data CLOB; buffer VARCHAR2(30000);
...
WHERE
TEMPLATE_TYPE_CODE=3 AND  FORM_ID = 'FDP_ATT_3A' FOR UPDATE;
buffer := 'rimeAdministrativeContact">
																								<xsl:for-each select="subcontract:RolodexDetails">
																							...
																							<xsl:for-each select="subcontract:PrimePrincipalInvestigator">
																								<xsl:for-each select="subcontract:FullName">
																									...
																									<xsl:choose>
																										...
																										<xsl:otherwise>
																											<fo:inline>
																												<xsl:copy-of select="$value-of-templat';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE data CLOB; buffer VARCHAR2(30000);
...
WHERE
TEMPLATE_TYPE_CODE=3 AND  FORM_ID = 'FDP_ATT_3A' FOR UPDATE;
buffer := ' select="$value-of-template"/>
																											</fo:block>
																										</xsl:when>
																										<xsl:otherwise>
																											<fo:inline>
...
WHERE
TEMPLATE_TYPE_CODE=3 AND  FORM_ID = 'FDP_ATT_3A' FOR UPDATE;
buffer := 'string($value-of-template),''&#x2029;'')">
																											<fo:block>
																												<xsl:copy-of select="$value-of-template"/>
																											</fo:block>
																										</xsl:when>
...
WHERE
TEMPLATE_TYPE_CODE=3 AND  FORM_ID = 'FDP_ATT_3A' FOR UPDATE;
buffer := ':copy-of select="$value-of-template"/>
																												</fo:block>
																											</xsl:when>
																											<xsl:otherwise>
																												<fo:inline>
...
END;
/
INSERT INTO SUBAWARD_FORMS(FORM_ID,DESCRIPTION,UPDATE_TIMESTAMP,UPDATE_USER,FORM,FILE_NAME,CONTENT_TYPE,VER_NBR,OBJ_ID,TEMPLATE_TYPE_CODE) values ('FDP_ATT_4','FDP Attachment 4',SYSDATE,'admin',EMPTY_CLOB(),'FDP Attachment 4.xsl','application/octet-stream',1,SYS_GUID(),3)
/
DECLARE data CLOB; buffer VARCHAR2(30000);
BEGIN
SELECT FORM INTO data FROM SUBAWARD_FORMS
WHERE
TEMPLATE_TYPE_CODE=3 AND  FORM_ID = 'FDP_ATT_4' FOR UPDATE;
buffer := '<?xml version="1.0" encoding="UTF-8"?>
<!--Designed and generated by Altova StyleVision Enterprise Edition 2008 rel. 2 - see http://www.altova.com/stylevision for more information.-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:award="http://subcontractFdpReports.bean.xml.utils.coeus.mit.edu/award" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:subcontract="http://subcontractFdpReports.bean.xml.utils.coeus.mit.edu/subcontract" xmlns:xdt="http://www.w3.org/2005/xpath-datatypes" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	<xsl:output version="1.0" method="xml" encoding="UTF-8" indent="no"/>
...
WHERE
TEMPLATE_TYPE_CODE=2 AND  FORM_ID = 'FDP_NIH' FOR UPDATE;
buffer := '												<xsl:text>The payment mechanism described in Article 22 and the financial reporting requirements in Article 52 of the Research Terms and Conditions and Article 8 of the Agency-Specific Requirements are replaced with Terms and Conditions (1) through (4) of this Subaward Agreement; and</xsl:text>
																				</fo:inline>
																			</fo:block>
																		</fo:list-item-body>
																	</fo:list-item>
															...
															<fo:block>
																...
																<xsl:choose>
																	...
																	<xsl:when test="subcontract:SubContractData/subcontract:SubcontractTemplateInfo/subcontract:TreatmentPrgmIncomeAdditive = &quot;N&quot;">
																		<fo:external-graphic content-height=';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE data CLOB; buffer VARCHAR2(30000);
...
WHERE
TEMPLATE_TYPE_CODE=2 AND  FORM_ID = 'FDP_ONR' FOR UPDATE;
buffer := '																<xsl:when test="subcontract:SubContractData/subcontract:SubcontractTemplateInfo/subcontract:AutomaticCarryForward = &apos;N&apos;">
																		<fo:inline>
																			<xsl:text>No</xsl:text>
																		</fo:inline>
																		<fo:block/>
