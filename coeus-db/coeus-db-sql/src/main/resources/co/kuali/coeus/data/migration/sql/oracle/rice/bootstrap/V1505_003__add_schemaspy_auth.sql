...
INSERT INTO KRIM_PERM_T (PERM_ID,PERM_TMPL_ID,NMSPC_CD,NM,DESC_TXT,ACTV_IND,OBJ_ID,VER_NBR)
  SELECT
    (SELECT KRIM_PERM_ID_S.NEXTVAL FROM dual),
    (SELECT PERM_TMPL_ID FROM KRIM_PERM_TMPL_T WHERE NMSPC_CD = 'KR-KRAD' AND NM = 'Open View'),
    'KC-SYS','Open Schema Spy View','Authorizes the opening of the Schema Spy View','Y',SYS_GUID(),1
  FROM dual
  WHERE NOT EXISTS (
    SELECT * FROM KRIM_PERM_T WHERE NMSPC_CD = 'KC-SYS' AND NM = 'Open Schema Spy View'
  );

INSERT INTO KRIM_PERM_ATTR_DATA_T (ATTR_DATA_ID,PERM_ID,KIM_TYP_ID,KIM_ATTR_DEFN_ID,ATTR_VAL,OBJ_ID,VER_NBR)
  SELECT
    (SELECT KRIM_ATTR_DATA_ID_S.NEXTVAL FROM dual),
    (SELECT KRIM_PERM_ID_S.CURVAL FROM dual),
  ...
  FROM dual
  WHERE NOT EXISTS (
    SELECT * FROM KRIM_PERM_ATTR_DATA_T WHERE ATTR_VAL = 'schemaspy'
  );
...
INSERT INTO KRIM_ROLE_PERM_T (ROLE_PERM_ID,ROLE_ID,PERM_ID,ACTV_IND,OBJ_ID,VER_NBR)
  ...
  FROM dual
  WHERE NOT EXISTS (
    SELECT * FROM KRIM_ROLE_PERM_T WHERE PERM_ID = (SELECT PERM_ID FROM KRIM_PERM_T WHERE NMSPC_CD = 'KC-SYS' AND NM = 'Open Schema Spy View')
  );
