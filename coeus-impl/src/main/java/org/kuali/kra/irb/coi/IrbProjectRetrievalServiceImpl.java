...
public class IrbProjectRetrievalServiceImpl extends AbstractMultiSponsorProjectRetrievalService {
    ...
    private static final String IRB_SPECIAL_VALUES_CRITERIA = Arrays.stream(ProtocolSpecialVersion.values())
            .map(v -> "PROTOCOL_NUMBER NOT LIKE '%" + v.getCode() + "%'")
            .collect(Collectors.joining(" AND "));

    private static final String IRB_ALL_HIGHEST_SEQUENCE_QUERY = "SELECT PROTOCOL_NUMBER, MAX(SEQUENCE_NUMBER) maxseq FROM PROTOCOL " +
            "WHERE " + IRB_SPECIAL_VALUES_CRITERIA + " " +
            "GROUP BY PROTOCOL_NUMBER";

    private static final String IRB_ALL_PROJECT_QUERY = "SELECT t.TITLE, t.PROTOCOL_ID, t.PROTOCOL_STATUS_CODE, t.INITIAL_SUBMISSION_DATE, t.EXPIRATION_DATE, t.PROTOCOL_NUMBER, t.SEQUENCE_NUMBER FROM (" + IRB_ALL_HIGHEST_SEQUENCE_QUERY + ") x " +
            "INNER JOIN PROTOCOL t ON t.PROTOCOL_NUMBER = x.PROTOCOL_NUMBER and t.SEQUENCE_NUMBER = x.maxseq";

    private static final String IRB_ALL_PROJECT_PERSON_QUERY = "SELECT t.PROTOCOL_ID, t.PERSON_ID, t.ROLODEX_ID, t.PROTOCOL_PERSON_ROLE_ID, t.PROTOCOL_NUMBER, t.SEQUENCE_NUMBER FROM (" + IRB_ALL_HIGHEST_SEQUENCE_QUERY + ") x " +
            "INNER JOIN PROTOCOL_PERSONS t ON t.PROTOCOL_NUMBER = x.PROTOCOL_NUMBER and t.SEQUENCE_NUMBER = x.maxseq";

    private static final String IRB_ALL_PROJECT_SPONSOR_QUERY = "SELECT t.PROTOCOL_ID, t.FUNDING_SOURCE, t.FUNDING_SOURCE_NAME, t.PROTOCOL_NUMBER, t.SEQUENCE_NUMBER FROM (" + IRB_ALL_HIGHEST_SEQUENCE_QUERY + ") x " +
            "INNER JOIN PROTOCOL_FUNDING_SOURCE t ON t.PROTOCOL_NUMBER = x.PROTOCOL_NUMBER and t.SEQUENCE_NUMBER = x.maxseq " +
            "WHERE t.FUNDING_SOURCE_TYPE_CODE = '" + FundingSourceType.SPONSOR + "'";

    private static final String IRB_PROJECT_QUERY = IRB_ALL_PROJECT_QUERY + " WHERE t.PROTOCOL_NUMBER = ?";
    private static final String IRB_PROJECT_PERSON_QUERY = IRB_ALL_PROJECT_PERSON_QUERY + " WHERE t.PROTOCOL_NUMBER = ?";
    private static final String IRB_PROJECT_SPONSOR_QUERY = IRB_ALL_PROJECT_SPONSOR_QUERY + " AND t.PROTOCOL_NUMBER = ?";
    ...
    protected String allProjectPersonQuery() {
        return IRB_ALL_PROJECT_PERSON_QUERY;
    }
