...
public class RequestDeactivateIacucProtocolUnavailableAuthorizer extends IacucProtocolAuthorizer {
    ...
    public boolean isAuthorized(String userId, IacucProtocolTask task) {        
        return ((isRequestForSuspension(findSubmisionHook(task.getProtocol()), IacucProtocolSubmissionType.REQUEST_SUSPEND) & !isAdmin(userId, RoleConstants.DEPARTMENT_ROLE_TYPE, RoleConstants.IACUC_ADMINISTRATOR)) ||
                !canExecuteAction(task.getProtocol(), IacucProtocolActionType.REQUEST_DEACTIVATE)
               )
               &&
