...
<xsl:stylesheet version="1.0"
 ...
 xmlns:Budget="http://apply.grants.gov/forms/Budget-V1.1"
 ...
 xmlns:glob="http://apply.grants.gov/system/Global-V1.0">
   ...
   <xsl:template match="Budget:BudgetNarrativeAttachments">
      <fo:root>
            ...
            <fo:flow flow-name="xsl-region-body">
               <fo:block>
                  ...
                  <fo:block space-before.optimum="1pt" space-after.optimum="2pt">
                     <fo:block>
                        <fo:inline font-size="inherited-property-value(&apos;font-size&apos;) + 4pt" font-weight="bold">Budget Narrative File(s)</fo:inline>
                     </fo:block>
                  </fo:block>
                  <fo:block>
                  ...
                  <xsl:for-each select="Budget:Attachments">
                     <xsl:for-each select="att:AttachedFile">
                        <xsl:if test="position()=1">
                           <fo:table height="600pt" width="100%" space-before.optimum="1pt" space-after.optimum="2pt">
                              <fo:table-column/>
                              <fo:table-column/>
                              <fo:table-header>
                                 <fo:table-row>
                                 ...
                                 <xsl:for-each select="../att:AttachedFile">
                                    <fo:table-row>
                                       <fo:table-cell padding-start="3pt" padding-end="3pt" padding-before="3pt" padding-after="3pt" display-align="center" text-align="start" border-style="solid" border-width="1pt" border-color="black" hyphenate="true" language="en">
                                          <fo:block>
                                             <xsl:for-each select="att:FileName"><fo:block/>
                                                <xsl:apply-templates/>
                                             </xsl:for-each>
                                          </fo:block>
                                       </fo:table-cell>
                                       <fo:table-cell padding-start="3pt" padding-end="3pt" padding-before="3pt" padding-after="3pt" display-align="center" text-align="start" border-style="solid" border-width="1pt" border-color="black" hyphenate="true" language="en">
                                          <fo:block>
                                             <xsl:for-each select="att:MimeType"><fo:block/>
                                                <xsl:apply-templates/>
                                             </xsl:for-each>
                                          </fo:block>
                                       </fo:table-cell>
