 ...
 */
package org.kuali.kra.s2s.rrbudget10;
...
import gov.grants.apply.forms.rrBudget1013V13.BudgetTypeDataType;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.Equipment;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.IndirectCosts;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.KeyPersons;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.OtherDirectCosts;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.OtherPersonnel;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.ParticipantTraineeSupportCosts;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.Travel;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.Equipment.EquipmentList;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.KeyPersons.KeyPerson;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.OtherPersonnel.GraduateStudents;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.OtherPersonnel.PostDocAssociates;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.OtherPersonnel.SecretarialClerical;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.OtherPersonnel.UndergraduateStudents;
import gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.ParticipantTraineeSupportCosts.Other;
import gov.grants.apply.forms.rrBudget1013V13.RRBudget1013Document.RRBudget1013.BudgetSummary;
...
/**
 * 
 * This class is to generate xml stream for grants.gov form RR_Budget10_1_3
 * ref schema namespace <code> http://apply.grants.gov/forms/RR_Budget10_1_3-V1.3</code>
 */
public class RRBudget10V1_3Generator extends RRBudgetBaseGenerator {

    private static final Log LOG = LogFactory.getLog(RRBudget10V1_3Generator.class);

    /**
     * This method returns RRBudget1013Document object based on proposal development
     * document which contains the informations such as
     * DUNSID,OrganizationName,BudgetType,BudgetYear and BudgetSummary.
     * 
     * @return rrBudgetDocument {@link XmlObject} of type RRBudget1013Document.
     */
    private RRBudget1013Document getRRBudget1013() {
        
        deleteAutoGenNarratives();
        RRBudget1013Document rrBudgetDocument = RRBudget1013Document.Factory
                .newInstance();
        RRBudget1013 rrBudget = RRBudget1013.Factory.newInstance();
        rrBudget.setFormVersion(S2SConstants.FORMVERSION_1_3);
        ...
        rrBudgetDocument.setRRBudget1013(rrBudget);
        return rrBudgetDocument;
    }
     ...
     *         object.
     */
    private void setBudgetYearDataType(RRBudget1013 rrBudget,BudgetPeriodInfo periodInfo) {
     ...
     * on BudgetSummaryInfo for the RRBudget1013.
     * 
     ...
     * based on the BudgetPeriodInfo for the RRBudget1013.
     * 
     * @param periodInfo
     *            (BudgetPeriodInfo) budget period entry.
     ...
     * BudgetYearDataType based on BudgetPeriodInfo for the RRBudget1013.
     * 
     * @param periodInfo
     *            (BudgetPeriodInfo) budget period entry.
    ...
    private void setOthersForOtherDirectCosts(OtherDirectCosts otherDirectCosts, BudgetPeriodInfo periodInfo) {
        if (periodInfo != null && periodInfo.getOtherDirectCosts() != null) {
            for (OtherDirectCostInfo otherDirectCostInfo : periodInfo.getOtherDirectCosts()) {
                gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.OtherDirectCosts.Other other = otherDirectCosts.addNewOther();
     ...
     * TotalOtherPersonnelFund based on BudgetPeriodInfo for the RRBudget1013.
     * 
     * @param periodInfo
     *            (BudgetPeriodInfo) budget period entry.
        ...
        if (periodInfo != null) {
            for (OtherPersonnelInfo otherPersonnelInfo : periodInfo
                ...
                if (OTHERPERSONNEL_POSTDOC.equals(otherPersonnelInfo
                    ...
                    gov.grants.apply.forms.rrBudget1013V13.BudgetYearDataType.OtherPersonnel.Other otherPersonnelDataType = otherPersonnel.addNewOther();
     ...
     * RRBudget1013,if it is a PostDocAssociates type.
     * 
     ...
     * NumberOfPersonnel,Compensation based on OtherPersonnelInfo for the
     * RRBudget1013, if it is a GraduateStudents type.
     * 
     * @param otherPersonnel
     *            (OtherPersonnelInfo) other personnel info entry.
     ...
     * NumberOfPersonnel,Compensation based on OtherPersonnelInfo for the
     * RRBudget1013,if it is a UndergraduateStudents type.
     * 
     * @param otherPersonnel
     *            (OtherPersonnelInfo) other personnel info entry.
     ...
     * NumberOfPersonnel,Compensation based on OtherPersonnelInfo for the
     * RRBudget1013,if it is a SecretarialClerical type.
     * 
     * @param otherPersonnel
     *            (OtherPersonnelInfo) other personnel info entry.
    ...
    /**
     * This method creates {@link XmlObject} of type {@link RRBudget1013Document} by
    ...
    public XmlObject getFormObject(XmlObject xmlObject) {
        RRBudget1013 rrBudget = (RRBudget1013) xmlObject;
        RRBudget1013Document rrBudgetDocument = RRBudget1013Document.Factory
                .newInstance();
        rrBudgetDocument.setRRBudget1013(rrBudget);
        return rrBudgetDocument;
