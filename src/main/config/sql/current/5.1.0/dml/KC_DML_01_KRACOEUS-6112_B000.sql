...
commit
/
INSERT INTO CUST_REPORT_DETAILS(REPORT_ID,REPORT_LABEL,REPORT_DESCRIPTION,REPORT_TYPE_CODE,RIGHT_REQUIRED,REPORT_DESIGN,FILE_NAME,CONTENT_TYPE,UPDATE_TIMESTAMP,UPDATE_USER,VER_NBR,OBJ_ID) values (SEQ_REPORT_ID.NEXTVAL,'proposalbycollege','proposalbycollege',(SELECT REPORT_TYPE_CODE FROM CUST_REPORT_TYPE WHERE REPORT_TYPE_DESC = 'Global'),(SELECT PERM_ID FROM KRIM_PERM_T WHERE NM = 'RUN GLOBAL REPORTS'),EMPTY_CLOB(),'proposalbycollege.rptdesign','application/octet-stream',SYSDATE,'admin',1,SYS_GUID())
/
