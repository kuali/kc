...
SELECT C.FINAL_SUBMISSION_DATE
FROM   AWARD_CLOSEOUT C
WHERE  C.CLOSEOUT_REPORT_CODE = '1';

CREATE VIEW VIEW_CO_D AS 
SELECT C.FINAL_SUBMISSION_DATE
FROM   AWARD_CLOSEOUT C
WHERE  C.CLOSEOUT_REPORT_CODE = '4';

CREATE VIEW VIEW_CO_E AS 
SELECT C.FINAL_SUBMISSION_DATE
FROM   AWARD_CLOSEOUT C
WHERE  C.CLOSEOUT_REPORT_CODE = '3';

CREATE VIEW VIEW_CO_F AS 
SELECT C.FINAL_SUBMISSION_DATE
FROM   AWARD_CLOSEOUT C
WHERE  C.CLOSEOUT_REPORT_CODE = '2';
...
FROM   AWARD_CLOSEOUT A,
       ...
       VIEW_CO_E E,
       VIEW_CO_F F
WHERE  A.AWARD_NUMBER = B.AWARD_NUMBER 
