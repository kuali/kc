...
select start_date, end_date, category, person_name, PERCENT_EFFORT, percent_charged,
 		ltrim(decode(ltrim(eb_rate), '0.00%', '', eb_rate)) eb_rate ,
		ltrim(decode(ltrim(vac_rate), '0.00%', ' ', vac_rate)) vac_rate, category_code, salary,
		cost_sharing_amount,fringe_cost_sharing,
 		fringe, PRINCIPAL_INVESTIGATOR, cost_element_desc
		...
		from (
			 select min(pd.start_date) start_date, max(pd.end_date) end_date, DECODE(inv.PRINCIPAL_INVESTIGATOR_FLAG,
				...
				--fn_get_sal_calc_cost_sharing(as_proposal_number,ai_period,max(pd.line_item_number),max(pd.person_number),max(pd.rate_number))+
				sum(pd.cost_sharing_amount) cost_sharing_amount,
				...
				decode(sum(eb.cost_sharing), NULL, 0, sum(eb.cost_sharing))+ decode(sum(vac.cost_sharing), NULL, 0, sum(vac.cost_sharing)) fringe_cost_sharing,
			...
			from osp$budget_per_details_for_edi pd,
					 ...
					 (select pcal.person_number person_num,
						 ...
						 pcal.calculated_cost_sharing cost_sharing
			...
			--	PERCENT_EFFORT, percent_charged,
				...
				eb.rate, vac.rate ,ce.DESCRIPTION,
