create or replace procedure dw_get_budget_la_exclusions
...
IF li_Count > 0 THEN             --LA applicable for this proposal
		open cur_summary for 
			select '1' sort_id, 'Allocated Administrative Support' description, sum(pd.salary_requested) cost
		...
		union
		select '2' sort_id, 'Total Employee Benefits' description, sum(pcal.calculated_cost) cost
		from osp$budget_per_details_for_edi pd, 
		...
		and bd.line_item_number not in (
			...
			and (rc.rate_class_type = 'L' or rc.rate_class_type = 'Y'))
		group by  ce.description || decode(bd.line_item_description, NULL, '', ' - ' || bd.line_item_description) 
		union
		select '4' sort_id, 'Allocated Lab Expense' description, sum(calamts.calculated_cost) cost
		from osp$budget_details bd, 
...
ELSE   
	--This proposal does not get open LA.
	--Open a dummy cursor which will never return a row.

	open cur_summary for 
			select '1' sort_id, 'Allocated Administrative Support' description, pd.salary_requested cost
			from osp$budget_per_details_for_edi pd
