CREATE OR REPLACE procedure get_budget_cum_oh_rate_base
...
-----------------------------------------------------------------------------------------------
select min(start_date) start_date, max(end_date) end_date, applied_rate, 
	   round(sum(base_cost),0) base_cost, round(sum(calculated_cost),0) calculated_cost, 
	   ON_OFF_CAMPUS_FLAG, rate_class_desc
from (select pd.start_date, pd.end_date, pcal.applied_rate,
	 ...
	 from osp$budget_per_details_for_edi pd,
		...
		osp$rate_class rc,
		(select pcal.BUDGET_PERIOD budget_per, 
		    pcal.person_number person_num,
			...
			and  pcal.proposal_number = as_proposal_number) eb,
		(select pcal.BUDGET_PERIOD budget_per,
			pcal.person_number person_num,
			pcal.rate_number rate_num,
			pcal.line_item_number line_num,
			...
			and  pcal.proposal_number = as_proposal_number) vac
