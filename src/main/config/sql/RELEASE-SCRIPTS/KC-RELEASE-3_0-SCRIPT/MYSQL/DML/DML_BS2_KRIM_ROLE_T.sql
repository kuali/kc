...
SELECT MAX(ID), 'ProtocolApprover', 'KC-PROTOCOL', 'This role exists primarily to grant implicit Cancel permission to Protocol Aggregators and Admins', 
(SELECT KIM_TYP_ID FROM KRIM_TYP_T WHERE NMSPC_CD = 'KC_SYS' AND NM = 'IRBApprover-Nested'), 'Y', NOW(), UUID(), 1 FROM KRIM_ROLE_ID_S;
