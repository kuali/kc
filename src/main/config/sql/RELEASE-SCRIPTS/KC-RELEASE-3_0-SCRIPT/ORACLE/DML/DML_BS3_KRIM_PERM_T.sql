...
INSERT INTO KRIM_PERM_T (PERM_ID, PERM_TMPL_ID, NM, DESC_TXT, ACTV_IND, NMSPC_CD, OBJ_ID)
VALUES (KRIM_PERM_ID_S.NEXTVAL, (SELECT PERM_TMPL_ID FROM KRIM_PERM_TMPL_T WHERE NM = 'Perform Document Action' AND NMSPC_CD = 'KC-IDM'), 'Protocol Review Not Required', 'permission for marking protocol as review not required', 'Y', 'KC-PROTOCOL', SYS_GUID () ) ;


-- Check for pre-existing permission
DECLARE
	DUMMY NUMBER;
BEGIN
	SELECT PERM_ID INTO DUMMY FROM KRIM_PERM_T WHERE NM = 'Populate KC Groups' AND NMSPC_CD = 'KC-SYS';
...
INSERT INTO KRIM_PERM_T (PERM_ID, OBJ_ID, VER_NBR, PERM_TMPL_ID, NMSPC_CD, NM, DESC_TXT, ACTV_IND)
VALUES (KRIM_PERM_ID_S.NEXTVAL, SYS_GUID(), 1, (SELECT T.PERM_TMPL_ID FROM KRIM_PERM_TMPL_T T WHERE T.NMSPC_CD = 'KC-IDM' AND T.NM = 'Perform Document Action') , 'KC-PROTOCOL', 'Modify Protocol Submission', 'modify an existing protocol submission', 'Y') ;
