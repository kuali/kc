INSERT INTO PROTO_CORRESP_TEMPL (PROTO_CORRESP_TEMPL_ID,PROTO_CORRESP_TYPE_CODE,COMMITTEE_ID,FILE_NAME,CORRESPONDENCE_TEMPLATE,UPDATE_USER,UPDATE_TIMESTAMP,OBJ_ID,VER_NBR)
    VALUES (SEQ_PROTO_CORRESP_TEMPL.NEXTVAL,(SELECT PROTO_CORRESP_TYPE_CODE FROM PROTO_CORRESP_TYPE WHERE DESCRIPTION = 'Approval Letter'),'DEFAULT','DEFAULT-1-ApprovalLetter.xsl',EMPTY_CLOB(),'admin',SYSDATE,SYS_GUID(),1)
/
DECLARE    data CLOB; buffer VARCHAR2(32000);
BEGIN
SELECT CORRESPONDENCE_TEMPLATE INTO data FROM PROTO_CORRESP_TEMPL WHERE PROTO_CORRESP_TYPE_CODE = (SELECT PROTO_CORRESP_TYPE_CODE FROM PROTO_CORRESP_TYPE WHERE DESCRIPTION = 'Approval Letter') AND COMMITTEE_ID = 'DEFAULT' FOR UPDATE;
...
BEGIN
SELECT CORRESPONDENCE_TEMPLATE INTO data FROM PROTO_CORRESP_TEMPL WHERE PROTO_CORRESP_TYPE_CODE = (SELECT PROTO_CORRESP_TYPE_CODE FROM PROTO_CORRESP_TYPE WHERE DESCRIPTION = 'Approval Letter') AND COMMITTEE_ID = 'DEFAULT' FOR UPDATE;
buffer := '<fo:table-body start-indent="0pt">
                                                <fo:table-row>
...
BEGIN
SELECT CORRESPONDENCE_TEMPLATE INTO data FROM PROTO_CORRESP_TEMPL WHERE PROTO_CORRESP_TYPE_CODE = (SELECT PROTO_CORRESP_TYPE_CODE FROM PROTO_CORRESP_TYPE WHERE DESCRIPTION = 'Approval Letter') AND COMMITTEE_ID = 'DEFAULT' FOR UPDATE;
buffer := '      <fo:table-cell padding="2pt" display-align="before">
                                                        <fo:block>
                                                            <fo:inline font-size="10pt" font-weight="bold">
...
buffer := '                                                       </fo:block>
                                ...
                                <xsl:choose>
                                    <xsl:when test="n1:Protocol/n1:Submissions/n1:CurrentSubmissionFlag = &apos;No&apos; and  n1:Protocol/n1:Submissions/n1:SubmissionDetails/n1:SubmissionTypeCode = 102">
                                        <fo:inline font-family="Arial" font-size="10pt">
                                            <xsl:text>The amendment to the above-referenced protocol has been A';
    DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE    data CLOB; buffer VARCHAR2(32000);
BEGIN
SELECT CORRESPONDENCE_TEMPLATE INTO data FROM PROTO_CORRESP_TEMPL WHERE PROTO_CORRESP_TYPE_CODE = (SELECT PROTO_CORRESP_TYPE_CODE FROM PROTO_CORRESP_TYPE WHERE DESCRIPTION = 'Approval Letter') AND COMMITTEE_ID = 'DEFAULT' FOR UPDATE;
buffer := 'PPROVED following full board review by the Committee on the Use of Humans as Experimental Subjects (COUHES).</xsl:text>
                                        </fo:inline>
                                        <fo:block/>
                                        <fo:inline font-family="Arial" font-size="10pt">
                                    ...
                                    <xsl:when test="n1:Protocol/n1:Submissions/n1:CurrentSubmissionFlag = &apos;No&apos; and  n1:Protocol/n1:Submissions/n1:SubmissionDetails/n1:SubmissionTypeCode = 101">
                                        ...
                                        <fo:block font-family="Arial" font-size="10pt" margin="0pt">
                                            <fo:block>
                                                ...
                                                <fo:block/>
                                                <fo:inline font-family="Arial" font-size="10pt">
                                                    <xsl:text>It is the Principal Investigator&apos;s responsibility to obtain review and continued approval before the expiration date.&#160; You may not continue any research activity beyond the expiration date without approval by COUHES.&#160; Failure to renew your study before the expiration date will result in termination of the';
    DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE    data CLOB; buffer VARCHAR2(32000);
BEGIN
SELECT CORRESPONDENCE_TEMPLATE INTO data FROM PROTO_CORRESP_TEMPL WHERE PROTO_CORRESP_TYPE_CODE = (SELECT PROTO_CORRESP_TYPE_CODE FROM PROTO_CORRESP_TYPE WHERE DESCRIPTION = 'Approval Letter') AND COMMITTEE_ID = 'DEFAULT' FOR UPDATE;
buffer := ' study and suspension of related research grants.</xsl:text>
                                                </fo:inline>
                                            </fo:block>
                                        </fo:block>
                                    </xsl:when>
...
BEGIN
SELECT CORRESPONDENCE_TEMPLATE INTO data FROM PROTO_CORRESP_TEMPL WHERE PROTO_CORRESP_TYPE_CODE = (SELECT PROTO_CORRESP_TYPE_CODE FROM PROTO_CORRESP_TYPE WHERE DESCRIPTION = 'Approval Letter') AND COMMITTEE_ID = 'DEFAULT' FOR UPDATE;
buffer := '           <xsl:for-each select="n1:Fullname">
                                                                                                                                                        <xsl:variable name="value-of-template">
                                                                                                                                                            <xsl:apply-templates/>
                                                                                                                                                        </xsl:variable>
                                                                                                                                                        <xsl:choose>
