...
declare
	l_new_seq INTEGER;
begin
	select nvl(max(to_number(ROLE_MBR_ID)),'1') + 1 into l_new_seq from KRIM_ROLE_MBR_T;
	...
	select nvl(max(to_number(PERM_ID)),'1') + 1 into l_new_seq from KRIM_PERM_T;
	execute immediate 'CREATE SEQUENCE KRIM_PERM_ID_BS_S START WITH ' || l_new_seq || 
		' MAXVALUE 99999999 MINVALUE 1 NOCYCLE NOCACHE NOORDER';

	select nvl(max(to_number(ROLE_PERM_ID)),'1') + 1 into l_new_seq from KRIM_ROLE_PERM_T;
	...
	select nvl(max(to_number(GRP_MBR_ID)),'1') + 1 into l_new_seq from KRIM_GRP_MBR_T;
	execute immediate 'CREATE SEQUENCE KRIM_GRP_MBR_ID_BS_S START WITH ' || l_new_seq || 
		' MAXVALUE 99999999 MINVALUE 1 NOCYCLE NOCACHE NOORDER';
		
	select nvl(max(to_number(ROLE_RSP_ID)),'1') + 1 into l_new_seq from KRIM_ROLE_RSP_T;
	execute immediate 'CREATE SEQUENCE KRIM_ROLE_RSP_ID_BS_S START WITH ' || l_new_seq || 
		' MAXVALUE 99999999 MINVALUE 1 NOCYCLE NOCACHE NOORDER';
		
	select nvl(max(to_number(ROLE_ID)),'1') + 1 into l_new_seq from KRIM_ROLE_T;
	execute immediate 'CREATE SEQUENCE KRIM_ROLE_ID_BS_S START WITH ' || l_new_seq || 
		' MAXVALUE 99999999 MINVALUE 1 NOCYCLE NOCACHE NOORDER';
		
	select nvl(max(to_number(ROLE_RSP_ACTN_ID)),'1') + 1 into l_new_seq from KRIM_ROLE_RSP_ACTN_T;
