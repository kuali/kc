...
/* select min(start_date) start_date, max(end_date) end_date, category, person_name, PERCENT_EFFORT, percent_charged,
	...
	*/
select start_date, end_date, category, person_name,
 		ltrim(decode(ltrim(eb_rate), '0.00%', '', eb_rate)) eb_rate ,
		ltrim(decode(ltrim(vac_rate), '0.00%', ' ', vac_rate)) vac_rate,
