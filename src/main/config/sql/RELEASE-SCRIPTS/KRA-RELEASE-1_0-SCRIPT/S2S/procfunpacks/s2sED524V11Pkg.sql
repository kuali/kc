...
procedure getBudgetYear (as_proposal_number in osp$budget.proposal_number%type,
 ...
 li_cost				number;
 li_direct_cost	number;
 li_total_direct  number;
 li_idc				number;
 li_cs				number;
 li_training_cost	number;
 li_training_cs	number;
...
/* get training costs - need to exclude from totals */
  ...
  select	   sum(decode(bd.line_item_cost,null,0,bd.line_item_cost))   ,
				sum(decode(bd.COST_SHARING_AMOUNT,null,0,bd.cost_sharing_amount))
...
li_cs := li_cs - li_training_cs;
li_total_direct := li_direct_cost - li_training_cost;

open cur_type for
  SELECT li_cost as total_cost,
         li_total_direct as total_direct_cost,
         li_idc as total_indirect_cost,
         li_cs  as cost_sharing
    		FROM  dual;
