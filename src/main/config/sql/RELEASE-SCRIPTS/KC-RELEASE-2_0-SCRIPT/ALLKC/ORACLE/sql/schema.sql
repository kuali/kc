...
DECLARE temp NUMBER;
BEGIN
	SELECT COUNT(*) INTO temp FROM user_sequences WHERE sequence_name = 'SEQ_PROPOSAL_NOTEPAD_ID';
	IF temp > 0 THEN EXECUTE IMMEDIATE 'DROP SEQUENCE SEQ_PROPOSAL_NOTEPAD_ID'; END IF;
END;
/

CREATE SEQUENCE SEQ_PROPOSAL_NOTEPAD_ID INCREMENT BY 1 START WITH 1 NOMAXVALUE NOCYCLE NOCACHE ORDER
/
