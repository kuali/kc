CREATE OR REPLACE procedure get_budget_cum_la_rate_base
...
-----------------------------------------------------------------------------------------------
  select rate_class, rate_type, min(start_date) start_date, max(end_date) end_date, applied_rate, round(sum(salary_requested),0) salary_requested, round(sum(calculated_cost),0) calculated_cost,
	...
	select rc.description rate_class, rt.description rate_type, pd.start_date, pd.end_date, pcal.applied_rate, to_number(pd.salary_requested) salary_requested, pcal.calculated_cost calculated_cost,
	...
	where pd.proposal_number = as_proposal_number and
			...
			rc.rate_class_type IN ('L', 'Y') and
			pcal.rate_class_code = rt.rate_class_code and
			pcal.rate_type_code = rt.rate_type_code
	union all
	select rc.description rate_class, rt.description rate_type, rb.start_date, rb.end_date, rb.applied_rate, to_number(rb.base_cost) salary_requested, rb.calculated_cost calculated_cost, rb.ON_OFF_CAMPUS_FLAG
   ...
   GROUP BY rate_class, rate_type, ON_OFF_CAMPUS_FLAG, applied_rate ORDER BY rate_class, rate_type, start_date, end_date;
