CREATE OR REPLACE package s2sPerfSitesV12Pkg as
...
procedure getPerfOrgId
		  (as_proposal_number IN osp$eps_proposal.proposal_number%type,
		   cur_type IN OUT result_sets.cur_generic) ;
           
procedure get_perf_sites (as_proposal_number IN OSP$EPS_PROPOSAL.PROPOSAL_NUMBER%TYPE ,
          cur_type  IN OUT RESULT_SETS.CUR_GENERIC );

procedure getCongDist
		  (as_proposal_number IN osp$eps_proposal.proposal_number%type,
           ai_site_number OSP$EPS_PROP_SITES.SITE_NUMBER%type,
		   cur_type IN OUT result_sets.cur_generic) ;
procedure getPerfOrgSiteNum
           (as_proposal_number IN osp$eps_proposal.proposal_number%type,
		   cur_type IN OUT result_sets.cur_generic) ;
procedure getDunsNumber
		  (as_org_id IN OSP$ORGANIZATION.ORGANIZATION_ID%type,
		   cur_type IN OUT result_sets.cur_generic) ;
procedure getIndivYNQ
          (as_organization_id IN osp$organization.organization_id%type,
           cur_type  IN OUT RESULT_SETS.CUR_GENERIC );
end;
/

CREATE OR REPLACE PACKAGE BODY s2sPerfSitesV12Pkg as

-------------------------------
-- procedure get_perf_sites
-------------------------------
procedure get_perf_sites (as_proposal_number IN OSP$EPS_PROPOSAL.PROPOSAL_NUMBER%TYPE ,
...
procedure getIndivYNQ
 ...
 begin
   	open cur_type for
			...
			select decode(answer, 'Y','Y: Yes','N','N: No') as INDIVIDUAL
			from osp$organization_ynq
			where organization_id = as_organization_id
			and 	question_id = '30';
...
begin
  open cur_type for
    ...
    where organization_id = as_org_id;
...
procedure getCongDist
  ...
  CURSOR c_congDist (prop_number    OSP$EPS_PROPOSAL.PROPOSAL_NUMBER%TYPE,
                     site_number    OSP$EPS_PROP_CONG_DISTRICT.SITE_NUMBER%type) is   
        select CONG_DISTRICT
        from   OSP$EPS_PROP_CONG_DISTRICT 
        where  PROPOSAL_NUMBER = prop_number
        and    SITE_NUMBER = ai_site_number;
 

  BEGIN
  
 	OPEN c_congDist (as_proposal_number, ai_site_number);
