...
VALUES ((SELECT (MAX(ID)) FROM SEQ_SPONSOR_FORM_TEMPLATES),(SELECT SPONSOR_FORM_ID FROM SPONSOR_FORMS WHERE PACKAGE_NAME = 'NIH 398 package (Coeus 4.0)'),17,'Resources','Resources.xslt','text/xml',EMPTY_CLOB(),'admin',NOW(),UUID(),1)
/
DECLARE data CLOB; buffer VARCHAR2(30000);
BEGIN
SELECT FORM_TEMPLATE INTO data FROM SPONSOR_FORM_TEMPLATES WHERE SPONSOR_FORM_ID = (SELECT SPONSOR_FORM_ID FROM SPONSOR_FORMS WHERE PACKAGE_NAME = 'NIH 398 package (Coeus 4.0)') AND PAGE_NUMBER = 17 FOR UPDATE;
...
<fo:table-row>
<fo:table-cell padding-bottom="0pt" padding-top="6pt" num';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE data CLOB; buffer VARCHAR2(30000);
BEGIN
SELECT FORM_TEMPLATE INTO data FROM SPONSOR_FORM_TEMPLATES WHERE SPONSOR_FORM_ID = (SELECT SPONSOR_FORM_ID FROM SPONSOR_FORMS WHERE PACKAGE_NAME = 'NIH 398 package (Coeus 4.0)') AND PAGE_NUMBER = 17 FOR UPDATE;
buffer := 'ber-columns-spanned="3" padding="0" display-align="center">
<fo:block>
<fo:inline font-family="Verdana" font-size="10pt" font-weight="bold">
...
<fo:table-row>
<fo:table-cell padding-bottom="0pt" padding-left="12pt" padding-top="0pt" number-columns-spanned=';
DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE data CLOB; buffer VARCHAR2(30000);
BEGIN
SELECT FORM_TEMPLATE INTO data FROM SPONSOR_FORM_TEMPLATES WHERE SPONSOR_FORM_ID = (SELECT SPONSOR_FORM_ID FROM SPONSOR_FORMS WHERE PACKAGE_NAME = 'NIH 398 package (Coeus 4.0)') AND PAGE_NUMBER = 17 FOR UPDATE;
buffer := '"3" padding="0" display-align="before">
<fo:block>
<xsl:for-each select="nih:ResearchAndRelatedProject">
<xsl:for-each select="Abstract">
<xsl:for-each select="AbstractText">
...
BEGIN
SELECT FORM_TEMPLATE INTO data FROM SPONSOR_FORM_TEMPLATES WHERE SPONSOR_FORM_ID = (SELECT SPONSOR_FORM_ID FROM SPONSOR_FORMS WHERE PACKAGE_NAME = 'NIH 398 package (Coeus 4.0)') AND PAGE_NUMBER = 17 FOR UPDATE;
buffer := ' <xsl:text>Resources Format Page</xsl:text>
</fo:inline>
</fo:block>
</fo:table-cell>
</fo:table-row>
