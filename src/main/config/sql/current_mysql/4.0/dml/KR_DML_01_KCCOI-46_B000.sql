...
((SELECT (MAX(ID)) FROM KRIM_RSP_ID_BS_S),(SELECT RSP_TMPL_ID FROM KRIM_RSP_TMPL_T WHERE NMSPC_CD = 'KR-WKFLW' AND NM = 'Review'), 'KC-WKFLW','COIReview','COI Disclosure Document - COIReview','Y',UUID(),1)
/
...
((SELECT (MAX(ID)) FROM KRIM_ATTR_DATA_ID_BS_S), (SELECT RSP_ID FROM KRIM_RSP_T WHERE NM = 'COIReview'),
...
VALUES ((SELECT (MAX(ID)) FROM KRIM_ATTR_DATA_ID_BS_S),(SELECT RSP_ID FROM KRIM_RSP_T WHERE NM = 'COIReview'),(SELECT KIM_TYP_ID FROM KRIM_TYP_T WHERE NMSPC_CD = 'KR-WKFLW' AND
...
INSERT INTO KRIM_RSP_ATTR_DATA_T (ATTR_DATA_ID, RSP_ID, KIM_TYP_ID, KIM_ATTR_DEFN_ID, ATTR_VAL, OBJ_ID) VALUES
((SELECT (MAX(ID)) FROM KRIM_ATTR_DATA_ID_BS_S), (SELECT RSP_ID FROM KRIM_RSP_T WHERE NM = 'COIReview'), (SELECT KIM_TYP_ID FROM KRIM_TYP_T WHERE NMSPC_CD = 'KR-WKFLW' AND
...
((SELECT (MAX(ID)) FROM KRIM_ROLE_RSP_ID_BS_S), UUID(), '1', (SELECT ROLE_ID FROM KRIM_ROLE_T WHERE ROLE_NM = 'COI Administrator') ,
...
((SELECT (MAX(ID)) FROM KRIM_ROLE_RSP_ACTN_ID_BS_S), UUID(), '1', 'A', '1', 'F', '*', (SELECT ROLE_RSP_ID FROM KRIM_ROLE_RSP_T WHERE ROLE_ID = (SELECT ROLE_ID FROM KRIM_ROLE_T WHERE ROLE_NM = 'COI Administrator') AND RSP_ID = (SELECT RSP_ID FROM KRIM_RSP_T WHERE NM = 'COIReview')),
'Y')
/
DELIMITER ;
