CREATE OR REPLACE procedure get_budget_cum_eb_rate_base
   ( as_proposal_number IN osp$budget_rate_base_for_edi.PROPOSAL_NUMBER%TYPE,
     cur_summary 	IN OUT result_sets.cur_generic) is

li_eb_on_la_rate_type_code         number;

begin

begin
	select to_number(value)
	into li_eb_on_la_rate_type_code
	from osp$parameter
	where parameter = 'EB_ON_LA_RATE_TYPE_CODE';
exception
	when others then
		li_eb_on_la_rate_type_code := 3;
end;
----------------------------------------------------------------------------------------------
...
open cur_summary for
  select rate_type, min(start_date) start_date, max(end_date) end_date, applied_rate, round(sum(salary_requested),0) salary_requested, round(sum(calculated_cost),0) calculated_cost,
	...
	where rb.proposal_number = as_proposal_number and
			...
			rc.rate_class_type = 'E' and
		   rb.rate_type_code <> li_eb_on_la_rate_type_code and
