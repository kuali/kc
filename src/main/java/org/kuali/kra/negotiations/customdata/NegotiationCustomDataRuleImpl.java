...
/**
 * This contains methods to process business rules on Negotiation Custom data.
 */
public class NegotiationCustomDataRuleImpl extends ResearchDocumentRuleBase implements NegotiationCustomDataRule {
    ...
    public boolean processSaveNegotiationCustomDataBusinessRules(NegotiationSaveCustomDataRuleEvent negotiationSaveCustomDataRuleEvent) {
        ...
        for (Map.Entry<String, CustomAttributeDocument> customAttributeDocumentEntry : customAttributeDocuments.entrySet()) {
            ...
            //set this value from negotiationCustomDataList so you have all of the data in one object to be passed to helper method.
            if(negotiationDocument.getNegotiation().getNegotiationCustomDataList().size() > 0) {
                int customAttributeId = customAttributeDocument.getCustomAttributeId();
                List<NegotiationCustomData> negotiationCustomDataList = negotiationDocument.getNegotiation().getNegotiationCustomDataList();
                for(NegotiationCustomData negotiationCustomData : negotiationCustomDataList){
                  if(negotiationCustomData.getCustomAttributeId() == customAttributeId){  
