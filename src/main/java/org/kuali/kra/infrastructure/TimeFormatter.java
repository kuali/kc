...
public class TimeFormatter extends Formatter
{
	...
	private static final Pattern TIME_PATTERN_12_HOUR =
		Pattern.compile("(0?[1-9]|1[0-2])" + // hour
						"(?:(?::|\\.)([0-5][0-9]))?" + // minute (optional)
						"(?:\\s*(([ap]m?|[ap]\\.m\\.)))?", // AM/PM (optional)
						Pattern.CASE_INSENSITIVE);
	private static final Pattern TIME_PATTERN_24_HOUR =
	        Pattern.compile("(0?[0-9]|1[0-9]|2[0-3])" + // hour
                    "(?:(?::|\\.)([0-5][0-9]))?", // minute (optional) 
                    Pattern.CASE_INSENSITIVE);
