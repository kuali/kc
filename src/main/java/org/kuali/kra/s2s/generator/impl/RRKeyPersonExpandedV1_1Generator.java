...
public class RRKeyPersonExpandedV1_1Generator extends
	...
	private static final Logger LOG = Logger
			.getLogger(RRKeyPersonExpandedV1_1Generator.class);
		...
		if (nKeyPersons.size() > 0) {
			for (ProposalPerson keyPerson : nKeyPersons) {
				...
				if (keyPerson.getProposalPersonRoleId().equals(CO_INVESTIGATOR)) {
					if (NIH.equals(pdDoc.getDevelopmentProposal()
							.getSponsorName())) {
						profileKeyPerson
								.setProjectRole(ProjectRoleDataType.PD_PI);
					} else {
						profileKeyPerson
								.setProjectRole(ProjectRoleDataType.CO_PD_PI);
					}
				} else {
					profileKeyPerson
