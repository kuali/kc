	...
	<table cellpadding=0 cellspacing="0" class="result-table">
		...
		<c:forEach items="${KualiForm.customDataHelper.customAttributeGroups[fullName]}" var="customAttributeDocument" varStatus="status">
			<tr class="datatable">
				...
				<td width="45%">
				<c:forEach var="negotiationCustomData" items="${KualiForm.document.negotiationList[0].negotiationCustomDataList}" > 
				  	<c:if test="${negotiationCustomData.customAttributeId == customAttributeDocument.customAttributeId}" >
				  	   <c:set var="customAttributeValue" value="${negotiationCustomData.value}" />
				  	</c:if>
				</c:forEach>
 			...
 			<c:forEach items="${ErrorPropertyList}" var="key">   
				...
				<c:choose>
                	...
                	<c:otherwise>
                	<kul:htmlControlAttribute property="${customAttributeId}" attributeEntry="${DataDictionary.NegotiationCustomData.attributes.value}"/>
