...
<div class="${responseDivClass}">
	<span class="Qresponse">
		<table style="border: none; width: 100%;" cellpadding="0"
			cellspacing="0">
			<tr>
				<td style="border: none;"><c:set var="answerLength"
						value="${question.answerMaxLength}" /> <c:choose>
						<c:when test="${answerLength > 80}">
                               ${bean.answerHeaders[answerHeaderIndex].answers[questionIndex].answer}

                        </c:when>
						<c:otherwise>
                           ${bean.answerHeaders[answerHeaderIndex].answers[questionIndex].answer}
                        </c:otherwise>
					</c:choose> <c:if test="${answerValidationError}">
