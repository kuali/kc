...
<div class="${responseDivClass}">
	<span class="Qresponse"> <c:set var="prop"
		...
		${kfunc:registerEditableProperty(KualiForm, prop)} <input
		type="hidden" name="checkboxToReset" value="${prop}" /> <c:if
			test="${bean.answerHeaders[answerHeaderIndex].answers[questionIndex].answer eq 'Y'}">
            Yes

            </c:if> <c:if
			test="${bean.answerHeaders[answerHeaderIndex].answers[questionIndex].answer eq 'N'}">
                    No
            </c:if> </span>
</div>
