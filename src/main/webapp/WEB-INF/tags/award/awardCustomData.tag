	...
	<table cellpadding=0 cellspacing="0" class="result-table">
		<c:forEach items="${KualiForm.customDataHelper.customAttributeGroups[fullName]}" var="customAttributeDocument" varStatus="status">
			<tr class="datatable">
				...
				<c:forEach var="awardCustomData" items="${KualiForm.document.awardList[0].awardCustomDataList}" > 
				  	<c:if test="${awardCustomData.customAttributeId == customAttributeDocument.customAttributeId}" >
				...
				<c:choose>
                	...
                	<c:otherwise>
                	<kul:htmlControlAttribute property="${customAttributeId}" attributeEntry="${DataDictionary.AwardCustomData.attributes.value}"/>
