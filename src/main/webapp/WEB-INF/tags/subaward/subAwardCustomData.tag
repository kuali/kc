	...
	<table cellpadding=0 cellspacing="0" class="result-table">
		<c:forEach items="${KualiForm.customDataHelper.customAttributeGroups[fullName]}" var="customAttributeDocument" varStatus="status">
			<tr class="datatable">
				...
				<c:forEach var="subAwardCustomData" items="${KualiForm.document.subAwardList[0].subAwardCustomDataList}" > 
				  	<c:if test="${subAwardCustomData.customAttributeId == customAttributeDocument.customAttributeId}" >
				  	   <c:set var="customAttributeValue" value="${subAwardCustomData.value}" />
				  	</c:if>
				</c:forEach>
				...
				<c:choose>
                	...
                	<c:otherwise>
                	<kul:htmlControlAttribute property="${customAttributeId}" attributeEntry="${DataDictionary.SubAwardCustomData.attributes.value}"/>
