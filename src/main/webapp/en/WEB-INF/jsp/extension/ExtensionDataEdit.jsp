...
<c:if test="${compareView}">
 <tr>
   <td class="datacell" colspan="3">
     <table align="center" width="100%" border="0" cellpadding="0" cellspacing="0" class="bord-r-t" style="margin: 0px">

     <!-- TODO Yuck, the following code is mostly duplicated from ExtensionData.jsp.  Once we move toward integration with the KNS, let's be sure to clean this up -->
     <c:forEach items="${existingExtensions.rows}" var="extensionsRows">
...
<c:choose>
	...
	<c:forEach items="${row.fields}" var="field">
	  ...
	  <c:if test="${drawFirstCell == true}">
		...
		<c:out value="${existingExtensions.data[field.propertyName]}"/>
	  </c:otherwise>
	</c:choose>
  </c:forEach>
</td>
...
<!-- End copy from ExtensionData.jsp -->
