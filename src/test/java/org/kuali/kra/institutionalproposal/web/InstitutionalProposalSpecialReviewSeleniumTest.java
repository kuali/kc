...
public class InstitutionalProposalSpecialReviewSeleniumTest extends KcSeleniumTestBase {
    ...
    public void testAddErrorSpecialReview() throws Exception {
        ...
        helper.click(ADD_SPECIAL_REVIEW_BUTTON);
        helper.assertErrorCount("tab-SpecialReview-div", 2);
    }
    
    private void setApprovedSpecialReviewFields() throws IOException {
        helper.set(HELPER_SPECIAL_REVIEW_TYPE_CODE_ID, SPECIAL_REVIEW_TYPE_ANIMAL_USAGE_NAME);
    ...
    private void checkTable(List<Review> reviews) {
        int index = 0;
        for (Review review : reviews) {
            helper.assertSelectedOptionsContains(String.format(LIST_SPECIAL_REVIEW_TYPE_CODE_ID, index), review.type);
            helper.assertSelectedOptionsContains(String.format(LIST_APPROVAL_TYPE_CODE_ID, index), review.approvalStatus);
            helper.assertElementContains(String.format(LIST_PROTOCOL_NUMBER_ID, index), review.protocolNumber);
            helper.assertElementContains(String.format(LIST_APPLICATION_DATE_ID, index), review.applicationDate);
            helper.assertElementContains(String.format(LIST_APPROVAL_DATE_ID, index), review.approvalDate);
            helper.assertElementContains(String.format(LIST_EXPIRATION_DATE_ID, index), review.expirationDate);
            for (String exemptionTypeCode : review.exemptionTypeCodes) {
                helper.assertSelectedOptionsContains(String.format(LIST_EXEMPTION_TYPE_CODES_ID, index), exemptionTypeCode);
            }
            helper.assertElementContains(String.format(LIST_COMMENTS_ID, index), review.comments);
            index++;
        }
    }
