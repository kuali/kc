...
/**
 * Test the Protocol Request Close Enrollment Authorizer.
 */
@RunWith(JMock.class)
public class ProtocolRequestCloseEnrollmentAuthorizerTest {
    ...
    private void runTest(final String protocolNumber, final boolean hasPermission, final boolean isActionAllowed, boolean expected) {
        ProtocolRequestCloseEnrollmentAuthorizer authorizer = new ProtocolRequestCloseEnrollmentAuthorizer();
        ...
        context.checking(new Expectations() {{
            allowing(actionService).isActionAllowed(ProtocolActionType.REQUEST_TO_CLOSE_ENROLLMENT, protocol); will(returnValue(isActionAllowed));
        }});
        authorizer.setProtocolActionService(actionService);
        
        ProtocolTask task = new ProtocolTask(TaskName.PROTOCOL_REQUEST_CLOSE_ENROLLMENT, protocol);
