 ...
 */
@RunWith(JMock.class)
public class ProtocolRequestCloseAuthorizerTest {
    ...
    private void runTest(final String protocolNumber, final boolean hasPermission, final boolean isActionAllowed, boolean expected) {
        ProtocolRequestCloseAuthorizer authorizer = new ProtocolRequestCloseAuthorizer();
        ...
        context.checking(new Expectations() {{
            allowing(actionService).isActionAllowed(ProtocolActionType.REQUEST_TO_CLOSE, protocol); will(returnValue(isActionAllowed));
        }});
        authorizer.setProtocolActionService(actionService);
        
        ProtocolTask task = new ProtocolTask(TaskName.PROTOCOL_REQUEST_CLOSE, protocol);
