 ...
 */
package org.kuali.kra.irb.actions.summaryhistory;
...
public class ProtocolSubmissionDetailsWebTest  extends CommitteeScheduleWebTestBase {
   ...
   // private static final String ADD_ROLE_BUTTON = "methodToCall.addCommitteeMembershipRole.document.committeeList[0].committeeMemberships[0].line0";
    ...
    /**
     * This method is to test the Submission Details.
     * Also set up protocol submission that submit to this committee
     * @throws Exception
     */
    @Test
    public void testSubmissionDetails() throws Exception {
...
//        com.gargoylesoftware.htmlunit.ScriptException: ReferenceError: "toggleTab" is not defined.
    ...
    /*
     * validate protocol submission details
     */
    private void  assertProtocolSubmiissionDetails(HtmlPage page)  throws Exception {
        // these tab is not open, so asText can't see it.
        String pageXml = page.asXml();
        int idx1 = pageXml.indexOf("Committee Id:");
        ...
        assertTrue(idx1 > 0);
        idx2 = pageXml.indexOf("Initial Protocol Application for Approval",idx1);        
        assertTrue(idx2 > 0);

        idx1 = pageXml.indexOf("Review Type:", idx2);
        assertTrue(idx1 > 0);
        idx2 = pageXml.indexOf("Expedited",idx1);        
        assertTrue(idx2 > 0);

        idx1 = pageXml.indexOf("Submission Status:", idx2);
        assertTrue(idx1 > 0);
        idx2 = pageXml.indexOf("Submitted to Committee",idx1);        
        assertTrue(idx2 > 0);

        idx2 = pageXml.indexOf("Joe  Tester", idx1); // not sure why 2 spaces ?
        assertTrue(idx2 > 0);
        idx1 = pageXml.indexOf("primary", idx2);
        assertTrue(idx1 > 0);
        idx2 = pageXml.indexOf("Ho, Pauline", idx1);
        assertTrue(idx2 > 0);
        idx1 = pageXml.indexOf("secondary", idx2);
        assertTrue(idx1 > 0);

        
        idx2 = pageXml.indexOf("Check List Items", idx1);
        assertTrue(idx2 > 0);
        idx1 = pageXml.indexOf("Clinical studies of drugs and medical devices only when condition", idx2);
        assertTrue(idx1 > 0);
    ...
    private HtmlPage getProtocolSubmission() throws Exception {
        ...
        setFieldValue(protocolActionsPage, "actionHelper.protocolSubmitAction.protocolReviewTypeCode", "2");
        ...
        setFieldValue(protocolActionsPage,"actionHelper.protocolSubmitAction.expeditedReviewCheckList[0].checked", "on");
