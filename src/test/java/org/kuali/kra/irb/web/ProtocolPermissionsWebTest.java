...
public class ProtocolPermissionsWebTest extends ProtocolWebTestBase {
    ...
    protected HtmlPage saveAndSearch(HtmlPage docPage) throws Exception {
        HtmlPage protocolPage = saveAndSearchDoc(docPage);
    ...
    /**
     * Test the initial creation of a protocol.  When a protocol is initially
...
//      assertEquals(UNIT_NUMBER, homeUnit);
     ...
     * of the protocol also reflects the deletion.  Beside the user table,
     ...
     * Make sure that a save and retrieval of the protocol shows that the
    ...
    public void testDeleteLastAggregator() throws Exception {
        ...
        List<String> errors = getErrors(permissionsPage, USER_TAB_DIV);
        assertEquals(errors.size(), 1);
        assertTrue(containsError(errors, "Must have at least one Protocol Aggregator"));
    }
    ...
    public void testEditAggregatorOnly() throws Exception {
        ...
        assertEquals(errors.size(), 1);
        assertTrue(containsError(errors, "Do not select other roles when Protocol Aggregator is selected"));
    }
    ...
    /**
     * Test a user who only has read only permission for a protocol.
     ...
     * the protocol and make sure those users are not there.
     * @throws Exception
     */
    @Test
    ...
    /**
     * Add some users to the protocol using the two standard roles and the unassigned role.
    ...
    /**
     * Add a single user to the protocol.
     * @param page
     * @param username
     * @param roleName
     * @return
    ...
    /**
     * Checks the Assigned Roles.  For each of the two protocol roles, verify that
