package org.kuali.coeus.org.jmock.lib.legacy;
...
public class ClassImposteriser implements Imposteriser {
    ...
    private static final CallbackFilter IGNORED_METHODS = method -> {
        if (method.isBridge())
            return 1;
    ...
    private Object proxy(Class<?> proxyClass, final Invokable mockObject) {
        ...
        proxy.setCallbacks(new Callback[] {
                (InvocationHandler) (receiver, method, args) -> mockObject.invoke(new Invocation(receiver, method, args)), NoOp.INSTANCE
        });
        return proxy;
    }
