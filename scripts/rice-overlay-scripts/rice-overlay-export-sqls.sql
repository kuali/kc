...
SELECT 'KC'||PRNCPL_ID AS PRNCPL_ID, 'kc'||PRNCPL_NM AS PRNCPL_NM, 'KC'||ENTITY_ID AS ENTITY_ID, PRNCPL_PSWD, ACTV_IND, OBJ_ID, VER_NBR FROM KRIM_PRNCPL_T WHERE ENTITY_ID > 10000 AND ENTITY_ID < 10083 
UNION 
...
SELECT 'KC'||ROLE_MBR_ID AS ROLE_MBR_ID, 
   ...
   CASE WHEN MBR_ID IN (SELECT PRNCPL_ID FROM KRIM_PRNCPL_T WHERE PRNCPL_NM IN ('quickstart', 'admin', 'kc', 'kr', 'notsys'))  
   THEN MBR_ID 
   ELSE 'KC'||MBR_ID END AS MBR_ID, 
